...
/// - `try_alloc` returns a valid, claimed slot pointer with value written and stamp set
/// - `dealloc` correctly returns the slot to the freelist
...
///
/// `Slot<A>` is 8 bytes (one pointer). The key is derived from the slot's
/// stamp field, so no separate key storage is needed.
///
...
impl<A: Alloc> Slot<A> {
    ...
    #[inline]
    pub fn key(&self) -> Key {
        // SAFETY: slot_ptr is valid (we own it)
        let slot = unsafe { &*self.slot_ptr };
        Key::new(slot.key_from_stamp())
    }
    ...
    pub fn into_inner(self) -> A::Item {
        ...
        // SAFETY: Value has been moved out, key is valid
