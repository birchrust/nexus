...
//! returned from any thread via `Drop` on `SingleAcquirerPooled`.
//!
...
impl<T> Drop for Inner<T> {
    fn drop(&mut self) {
        ...
        // Values that are "out" (held by SingleAcquirerPooled) have been moved
...
// =============================================================================
// BoundedSingleAcquirerPool - the pool itself
// =============================================================================
...
/// }).join().unwrap();
/// ```
pub struct BoundedSingleAcquirerPool<T> {
    inner: Arc<Inner<T>>,
}

impl<T> BoundedSingleAcquirerPool<T> {
    ...
    /// Converts the pool into an `Acquirer`.
    ///
    /// The `Acquirer` is the only handle that can acquire items from the pool.
    /// Items are returned automatically when `SingleAcquirerPooled` guards are dropped.
    pub fn into_acquirer(self) -> SingleAcquirer<T> {
        SingleAcquirer { inner: self.inner }
    }
}

// =============================================================================
// Acquirer - the acquire-side handle
// =============================================================================

/// Handle for acquiring items from the pool.
///
/// Only one `Acquirer` exists per pool. It cannot be cloned or shared
/// across threads (it is `Send` but not `Sync` or `Clone`).
///
/// When the `Acquirer` is dropped, outstanding `SingleAcquirerPooled` guards
/// will drop their values directly instead of returning them to the pool.
pub struct SingleAcquirer<T> {
    inner: Arc<Inner<T>>,
}

// Acquirer is Send (can be moved to another thread) but not Sync (not shared)
// Not Clone - only one acquirer exists
unsafe impl<T: Send> Send for SingleAcquirer<T> {}

impl<T> SingleAcquirer<T> {
    /// Attempts to acquire an object from the pool.
    ///
    /// Returns `None` if all objects are currently in use.
    #[inline]
    pub fn try_acquire(&self) -> Option<SingleAcquirerPooled<T>> {
...
// =============================================================================
// SingleAcquirerPooled - RAII guard
// =============================================================================
...
pub struct SingleAcquirerPooled<T> {
    value: ManuallyDrop<T>,
    idx: u32,
    inner: Weak<Inner<T>>,
}

// SingleAcquirerPooled is Send + Sync - can be sent anywhere, dropped from anywhere
unsafe impl<T: Send> Send for SingleAcquirerPooled<T> {}
unsafe impl<T: Send + Sync> Sync for SingleAcquirerPooled<T> {}

impl<T> Deref for SingleAcquirerPooled<T> {
    type Target = T;
...
impl<T> DerefMut for SingleAcquirerPooled<T> {
    #[inline]
...
impl<T> Drop for SingleAcquirerPooled<T> {
    fn drop(&mut self) {
...
mod tests {
    ...
    fn basic_acquire_release() {
        let pool =
            BoundedSingleAcquirerPool::new(3, || Vec::<u8>::with_capacity(16), |v| v.clear());
    ...
    fn cross_thread_return() {
        let pool = BoundedSingleAcquirerPool::new(2, || 42u32, |_| {});
    ...
    fn acquirer_dropped_first() {
        let item;
        {
            let pool = BoundedSingleAcquirerPool::new(1, || String::from("test"), |s| s.clear());
    ...
    #[test]
    fn lifo_ordering() {
        let pool = BoundedSingleAcquirerPool::new(3, || Vec::<u8>::new(), |v| v.clear());
    ...
    fn zero_capacity_panics() {
        let _ = BoundedSingleAcquirerPool::new(0, || (), |_| {});
    }
    ...
    fn stress_single_thread() {
        let pool =
            BoundedSingleAcquirerPool::new(100, || Vec::<u8>::with_capacity(64), |v| v.clear());
        ...
        thread::scope(|s| {
            ...
            s.spawn(move || {
                while let Ok(item) = rx.recv() {
                    let _item: SingleAcquirerPooled<AtomicUsize> = item;
    ...
    fn stress_concurrent_return() {
        ...
        let pool = BoundedSingleAcquirerPool::new(1000, || 0u64, |_| {});
