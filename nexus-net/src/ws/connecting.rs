...
impl WsStreamBuilder {
    ...
    pub fn begin_connect<S: Read + Write>(
        ...
        if matches!(initial_state, ConnectState::HttpSend) {
            connecting.prepare_http_request(parsed.path);
        }
...
impl<S: Read + Write> WsConnecting<S> {
    ...
    fn finish(&mut self) -> WsStream<S> {
        // Use a dummy stream to swap out — we're consuming self logically
        // but can't move out of &mut self. Use ManuallyDrop trick.
        ...
        // We need to move stream and tls out. Since poll() returns
        // Option<WsStream>, and we only call finish() once (state=Done),
        // we use unsafe to move out of the mutable borrow.
        //
        // SAFETY: finish() is only called once when state==Done.
        // After this, self is in an unspecified state and must not be used.
        unsafe {
            let stream = std::ptr::read(&raw const self.stream);
            #[cfg(feature = "tls")]
            let tls = std::ptr::read(&raw const self.tls);
    ...
    fn read_bytes(&mut self, dst: &mut [u8]) -> Result<usize, WsError> {
        ...
        if let Some(tls) = &mut self.tls {
            return match tls.read_tls_from(&mut self.stream) {
                ...
                Ok(_) => {
                    tls.process_new_packets()?;
                    Ok(tls.read_plaintext(dst).unwrap_or(0))
                }
