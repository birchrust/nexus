//! Ping-pong latency benchmark for crossbeam-channel
//!
//! Measures round-trip latency with exactly one message in flight.
//!
//! Run: cargo build --release --bench perf_crossbeam_channel_latency
//! Profile: sudo taskset -c 0,2 ./target/release/deps/perf_crossbeam_channel_latency-*
...
fn main() {
    let (tx_fwd, rx_fwd) = bounded::<u64>(CAPACITY);
    let (tx_ret, rx_ret) = bounded::<u64>(CAPACITY);
    ...
    println!(
        "crossbeam channel latency (cycles): min={} p50={} p99={} p99.9={} max={}",
