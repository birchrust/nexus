...
macro_rules! impl_moments_float {
    ($name:ident, $ty:ty) => {
        ...
        #[doc = concat!("for i in 1..=1000u64 { m.update(i as ", stringify!($ty), "); }")]
        ...
        impl $name {
            ...
            pub fn update(&mut self, sample: $ty) {
                ...
                self.m3 +=
                    term1 * delta_n * (n - 2.0 as $ty) - 3.0 as $ty * delta_n * self.m2;
                self.m2 += term1;
                self.mean += delta_n;
            }
            ...
            pub fn merge(&mut self, other: &Self) {
                ...
                let new_m4 = self.m4 + other.m4
                    ...
                    + 6.0 as $ty * delta2 * (n_a * n_a * other.m2 + n_b * n_b * self.m2)
                        / (n * n)
...
mod tests {
    ...
    fn uniform_1_to_100() {
        ...
        assert!(
            (pop_var - 833.25).abs() < 1e-6,
            "pop variance = {pop_var}"
        );
    ...
    fn priming_thresholds() {
        ...
        m.update(4.0);
        assert!(m.kurtosis().is_some()); // 4 samples
        assert!(m.is_primed());
    }
